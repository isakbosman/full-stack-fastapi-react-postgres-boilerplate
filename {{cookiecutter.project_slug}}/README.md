...
```
FRONTEND_APP_DOMAIN_DEV=localhost
```

* Change that line to the domain you are going to use, e.g.:

```
FRONTEND_APP_DOMAIN_DEV=localhost.tiangolo.com
```

That variable will make your frontend communicate with that domain when interacting with your backend API, when the other variable `FRONTEND_APP_ENV` is set to `development`.
